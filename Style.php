...
class Style {
    ...
    private function getStyles() {
        ...
        $dir = new QDir(self::$path);
        /** Инициализируем класс QFile */
        $file = new QFile();
        ...
        if($dir->exists()) {
            ...
            foreach($themes as $theme) {
                ...
                $file->setFileName(self::$path."/$theme/about.json");
                if($file->exists()) {
                    $file->open(QFile::ReadOnly);
                    self::$about[$theme] = json_decode($file->readAll(), true);
                    $file->close();
                }
            }
        }
    ...
    private function copyDefault() {
        ...
        $default = json_decode(file_get_contents("qrc://$this->resource/styleSheet.json"), true);
        /** Проверяем данные из файла на пустоту */
        if(!empty($default)) {
            /** Задаем путь к директории с темами приложения */
            $dir = new QDir(self::$path);
            /** Инициализируем объект QFile */
            $file = new QFile();
            /** Проходим по массиву данных */
            foreach($default as $theme => $files) {
                /** Задаем путь к диретории темы оформления */
                $dir->setPath(self::$path."/$theme");
                /** Проверяем директорию на существование, если ее нет то создаем ее */
                if(!$dir->exists()) $dir->mkdir(self::$path."/$theme");
                /** Проходим по списку файлов темы оформления */
                foreach($files as $filename) {
                    /** Задаем путь к файлу */
                    $file->setFileName(self::$path."/$theme/$filename");
                    ...
                    if($file->exists()) {
                        ...
                        if($resHash !== $appHash) {
                            ...
                            $file->write(file_get_contents("qrc://$this->resource/$theme/$filename"));
                            $file->close();
                        }
                    } else {
    ...
    static public function setTheme($name) {
        ...
        if(is_string($name) && !empty($name)) {
            ...
            $dir = new QDir(self::$tree[$name]);
            /** Проверяем директорию на существование */
            if($dir->exists()) {
                $file = new QFile();
                /** Получаем списко файлов с расширением .qss */
                $styles = $dir->entryList(QDir::Files | '*.qss');
                /** Обрабатываем список файлов */
                foreach($styles as $style) {
                    /** Задаем путь к файлу для работы */
                    $file->setFileName(self::$tree[$name]."/$style");
                    /** Открываем файл только для чтения */
                    $file->open(QFile::ReadOnly);
                    /** Записываем данные из файла в массив темы */
                    self::$styles[$name][str_replace('.qss', '', $style)] = $file->readAll();
                    /** Закрываем файл */
                    $file->close();
                }
                /** Освобождаем временные переменные */
